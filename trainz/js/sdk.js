...
export function bindSdkStatus(onStatus) {
  sdk.onConnectionState((event) => onStatus(Boolean(event.connected), event.state || "disconnected"));
}
...
export function bindSwarmCommands(onCommand) {
  sdk.onMessage((event) => {
    ...
    if (match) onCommand({ command: match[1].toLowerCase(), argument: match[2] || "", event });
  });
}
